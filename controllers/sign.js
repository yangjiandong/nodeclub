...
exports.signup = function(req, res, next) {
  ...
  if (method == 'post') {
    ...
    User.find({
          ...
          pass = md5(pass);
          // create gavatar
          var avatar_url = 'http://www.gravatar.com/avatar/' + md5(email)
              + '?size=48';
          ...
          user.save(function(err) {
                if (err)
                  return next(err);
                mail_ctrl.send_active_mail(email, md5(email
                        + config.session_secret), name, email, function(err,
                        success) {
                      if (success) {
                        res.render('sign/signup', {
  ...
  if (method == 'post') {
    ...
    User.findOne({
          ...
          user.save(function(err) {
                ...
                mail_ctrl.send_reset_pass_mail(email, retrieveKey, user.name,
...
/**
 * reset password 'get' to show the page, 'post' to reset password after reset
 * password, retrieve_key&time will be destroy
 *
 * @param {http.req}
 *          req
 * @param {http.res}
